...
function convertStep (stepJson, scenarioJson, options) {
    ...
    if (result.duration) {
        ...
        stepOutput[0]._attr.time = result.duration / 1000;
    } else {
        //for RP 3
        stepOutput[0]._attr.time = 0;
    }
...
function convertFeature(featureJson, options) {
    ...
    if(featureJson.properties) {
        for (var propertyName in featureJson.properties) {
            if (featureJson.properties.hasOwnProperty(propertyName)) {
                featureOutput[1].properties.push(createProperty(
                    propertyName, featureJson.properties[propertyName]
                ));
            }
        }
    }
    if(featureJson.elements) {
        featureJson.elements
            ...
            .map(function (scenarioJson) {
                ...
                featureOutput[0]._attr.scenarios += 1;
                var scenario = convertScenario(scenarioJson, options);
                if (scenario[1] && scenario[1].failure) {
                    featureOutput[0]._attr.failures += 1;
                }
                if (scenario[1] && scenario[1].skipped) {
...
// commented for adding 2/3 level option like this: --scope=scenario, default --scope=step
